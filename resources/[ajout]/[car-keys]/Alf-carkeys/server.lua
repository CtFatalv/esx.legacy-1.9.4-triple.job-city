...
AddEventHandler('Alf-Carkeys:deleteKey', function(plate, id)
    local xPlayer = ESX.GetPlayerFromId(source)
    MySQL.Async.execute("DELETE FROM `vehicle_keys` WHERE identifier = @identifier AND id = @id AND plate = @plate AND state = 'Copie' OR identifier = @identifier AND id = @id AND plate = @plate AND state = 'Garagenzugriff'", { 
        ['@identifier'] = xPlayer.identifier,
        ['@plate'] = plate,
        ['@id'] = id
    })
...
ESX.RegisterServerCallback('Alf-Carkeys:getPlayersKeysCopy', function(source, cb, id, plate, state)
    local xPlayer = ESX.GetPlayerFromId(source)
    MySQL.Async.fetchAll('SELECT * FROM vehicle_keys WHERE identifier = @identifier AND state="Copie"', {
        ['@identifier'] = xPlayer.identifier
    }, function(keys)
        if keys ~= nil then
            cb(keys)
