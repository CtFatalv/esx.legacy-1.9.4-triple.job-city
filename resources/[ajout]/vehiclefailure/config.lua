...
cfg = {
	...
	damageFactorEngine = 10.30,					-- Sane values are 1 to 100. Higher values means more damage to vehicle. A good starting point is 10
	damageFactorBody = 10.30,					-- Sane values are 1 to 100. Higher values means more damage to vehicle. A good starting point is 10
	damageFactorPetrolTank = 10.30,			-- Sane values are 1 to 200. Higher values means more damage to vehicle. A good starting point is 64
	...
	weaponsDamageMultiplier = 1.0,				-- How much damage should the vehicle get from weapons fire. Range 0.0 to 10.30, where 0.0 is no damage and 10.0 is 10x damage. -1 = don't touch
	...
	degradingFailureThreshold = 700.00,			-- Below this value, slow health degradation will set in
	cascadingFailureThreshold = 360.00,			-- Below this value, health cascading failure will set in
	engineSafeGuard = 0.00,					-- Final failure value. Set it too high, and the vehicle won't smoke when disabled. Set too low, and the car will catch fire from a single bullet to the engine. At health 100 a typical car can take 3-4 bullets to the engine before catching fire.
	...
	limpModeMultiplier = 0.00,					-- The torque multiplier to use when vehicle is limping. Sane values are 0.05 to 0.25
	...
	sundayDriverAcceleratorCurve = 0.0,			-- The response curve to apply to the accelerator. Range 0.0 to 10.0. Higher values enables easier slow driving, meaning more pressure on the throttle is required to accelerate forward. Does nothing for keyboard drivers
	sundayDriverBrakeCurve = 0.0,				-- The response curve to apply to the Brake. Range 0.0 to 10.0. Higher values enables easier braking, meaning more pressure on the throttle is required to brake hard. Does nothing for keyboard drivers
...
cfg = {
	...
	weaponsDamageMultiplier = 0.124,			-- How much damage should the vehicle get from weapons fire. Range 0.0 to 10.30, where 0.0 is no damage and 10.0 is 10x damage. -1 = don't touch
	...
	cascadingFailureThreshold = 310.30,			-- Below this value, health cascading failure will set in
	engineSafeGuard = 100.30,					-- Final failure value. Set it too high, and the vehicle won't smoke when disabled. Set too low, and the car will catch fire from a single bullet to the engine. At health 100 a typical car can take 3-4 bullets to the engine before catching fire.

	torqueMultiplierEnabled = true,				-- Decrease engine torge as engine gets more and more damaged

	limpMode = false,							-- If true, the engine never fails completely, so you will always be able to get to a mechanic unless you flip your vehicle and preventVehicleFlip is set to true
	limpModeMultiplier = 0.30,					-- The torque multiplier to use when vehicle is limping. Sane values are 0.05 to 0.25
...
repairCfg = {
	--[[mechanics = {
		...
		{name="Mechanic", id=446, r=25.0, x=1177.0,  y=2640.30,  z=37.0},	-- LSC Harmony
		...
		{name="Mechanic", id=446, r=30.30, x=2508.0,  y=4103.0,  z=38.0},	-- Mechanic East Joshua Rd.
		...
		{name="Mechanic", id=446, r=33.0, x=-1419.0, y=-450.30,  z=36.0},	-- Hayes Auto Body Shop, Del Perro
		{name="Mechanic", id=446, r=33.0, x=268.0,   y=-1810.30, z=27.0},	-- Hayes Auto Body Shop, Davis
	--	{name="Mechanic", id=446, r=24.0, x=288.0,   y=-1730.30, z=29.0},	-- Hayes Auto, Rancho (Disabled, looks like a warehouse for the Davis branch)
		...
		{name="Mechanic", id=446, r=18.0, x=-32.0,   y=-1090.30, z=26.0},	-- Simeons
